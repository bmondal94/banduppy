...
```
    SimulationParentFolder = '/local/GitHub/TestBandUPpy/Vasp/Si' 
    # supercell dimension 
...
```
    ...
    = band_unfold.Unfold(ab_initio_code='vasp', 
                         ...
                         #SBZ_PBZ_kpts_map={k:SBZ_PBZ_kpts_mapping[k] for k in range(1,60)}, # Allow unfolding selected K-points
...
```
    fig, ax, CountFig \
    = plot_unfold.plot_ebs(unfolded_kpoints=kpline1, 
    ...
    fig, ax, CountFig \
    = plot_unfold.plot_ebs(ax=ax, unfolded_kpoints=kpline1, 
...
```
    plot_unfold.plot_scf(unfolded_kpoints=kpline, unfolded_bandstructure=unfolded_bandstructure_,
